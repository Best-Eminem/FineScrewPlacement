...
def my_func(x1, y1,env):
    ...
    return env.simulate_volume(radian_L)
...
def get_random_horizon():
    horizon = random.uniform(-0.53, 0.53)
    while(horizon < -0.53 or horizon > 0.53):
        horizon = random.uniform(-0.53, 0.53)
    return horizon

def get_random_sagittal():
    sagittal = random.uniform(-1.1, 1.1)
    while(sagittal < -1.1 or sagittal > 1.1):
        sagittal = random.uniform(-1.1, 1.1)
    return sagittal
